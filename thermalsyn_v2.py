...
def low_freq_jpl_correction(x,Theta,p,derivative=False,GRB_convention=False,gamma_m=np.nan):
    ...
    if GRB_convention==True:
        zeta_e = 1
        mu, mu_e = 1,1
        gamma_m = (eps_e/zeta_e)*((p-2.0)/(p-1.0))*e/(n*C.me*C.c*C.c)
...
def low_freq_apl_correction(x,Theta,p,derivative=False,GRB_convention=False,gamma_m=np.nan):
...
def jnu_pl(x,n,B,Theta,delta=1e-1,p=3.0,GRB_convention=False,z_cool=np.inf,gamma_m=np.nan):
    ...
    val *= low_freq_jpl_correction(x,Theta,p,gamma_m=gamma_m,GRB_convention=GRB_convention)
    # fast-cooling correction:
    z0 = x**0.5
    val *= np.minimum( 1e0, z_cool/z0 )
    return val

def alphanu_pl(x,n,B,Theta,delta=1e-1,p=3.0,GRB_convention=False,z_cool=np.inf,gamma_m=np.nan):
    ...
    # correct emission at low-frequencies x < x_m:
    val *= low_freq_apl_correction(x,Theta,p,gamma_m=gamma_m,GRB_convention=GRB_convention)
    # fast-cooling correction:
    z0 = x**0.5
    val *= np.minimum( 1e0, z_cool/z0 )
    return val
