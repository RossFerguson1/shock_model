...
"""
Code necessary to run Figure 2 in Ferguson and Margalit (2025)
"""
...
'''
Functions used to calculate j_eff below; see Equation 18 in Ferguson & Margalit (2025)
'''
...
for col in range(cols):
        ...
        for j in range(len(x)):
                ...
                for m in range(len(y)):
                    D_calc = flux_variables.D(np.abs(x[j]),y[m],T,n0,R_l[i],X_perp[i],t_test[i], R_test[i], BG[i],k[i],alpha[i],z,R0[i],GRB_convention=GRB_convention)
                    nu_prime = nu/D_calc
                    if flux_variables.j_nu_prime(nu_prime, flux_variables.xi(np.abs(x[j]),y[m],T, R_l[i], X_perp[i], 
...
#Full calculation of j_eff                        
                        j_eff_val = j_eff(np.abs(x[j]),y[m],nu,i)
                        Z[m,j] = j_eff_val
...
cbar.set_label(r'$j_{\rm eff}/j_{\rm eff, max}$',fontsize=16)
cbar.formatter = FormatStrFormatter("%.2f") 
cbar.update_ticks()    
plt.show()
fig.savefig(os.getcwd()+"/Fig2.pdf",bbox_inches='tight')